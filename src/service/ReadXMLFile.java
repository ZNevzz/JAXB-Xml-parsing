...
//		/*
		File file=new File(BOOKSTORE_XML);
		...
		try {
			...
			feedbacks=xmlDatabase.getFeedbacks();
			
			for(Feedback i: feedbacks){
				
				companies=i.getCompanies();
				
				for(Company j: companies){
					
					System.out.println(j.getCompanyId()+" "+j.getCompanyName());
					
					users=j.getUsers();
					
					for(User k:users){
						
						System.out.println(k.getUserBranch()+" "+k.getUsername()+" "+k.getUserRole());
						System.out.println(k.getFeedback());
					}
					
				}
			}
			
			user1.setUsername("7099");
			user1.setUserRole("student");
			user1.setUserBranch("ELEX");
			user1.setFeedback("Very bad experienece of JP morgan dude!!!");
			...
			marshaller.marshal(xmlDatabase, System.out);
			marshaller.marshal(xmlDatabase, new File(BOOKSTORE_XML));
